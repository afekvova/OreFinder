package ru.afek.orefinder.config;
...
import ru.afek.orefinder.OreFinder;
import ru.afek.orefinder.utils.StringHelper;

public class Config {

	private static Logger logger = OreFinder.getInstance().getLogger();
	...
	public static <T> T get(String key, Class root) {
		...
		logger.info("Failed to get config option: " + key);
		return null;
	}
		...
		if (instance != null) {
			...
			if (field != null) {
				try {
					...
					logger.info("Invalid configuration value: " + key + ": "
							+ value + " in " + root.getSimpleName());
					e.printStackTrace();
				}
			}
		}
		logger.info("Failed to set config option: " + key + ": " + value + " | "
				+ instance);
	}
	...
	private static String toYamlString(Object value, String spacing) {
		if (value instanceof List) {
			...
			for (Object obj : listValue) {
				m.append(System.lineSeparator() + spacing + "- "
						+ toYamlString(obj, spacing));
			}
			return m.toString();
		}
	...
	private static void save(PrintWriter writer, Class clazz, Object instance,
			int indent) {
		try {
			String lineSeparator = System.lineSeparator();
			String spacing = StringHelper.repeat(" ", indent);
			for (Field field : clazz.getFields()) {
				...
				if (comment != null) {
					for (String commentLine : comment.value()) {
						writer.write(
								spacing + "# " + commentLine + lineSeparator);
					}
				}
				...
				if (create != null) {
					...
					if (value == null && field.getType() != ConfigBlock.class) {
						...
						for (Class current : classes) {
							if (StringHelper.isEqual(current.getSimpleName(),
									field.getName())) {
					...
					writer.write(spacing + toNodeName(field.getName() + ": ")
			...
			for (Class<?> current : clazz.getClasses()) {
				if (current.isInterface()
						|| current.getAnnotation(Ignore.class) != null) {
					continue;
				}
				if (indent == 0) {
				...
				writer.write(spacing + toNodeName(current.getSimpleName()) + ":"
						+ lineSeparator);
				BlockName blockNames = current.getAnnotation(BlockName.class);
				if (blockNames != null) {
					Field instanceField = clazz.getDeclaredField(
							toFieldName(current.getSimpleName()));
					setAccessible(instanceField);
					ConfigBlock value = (ConfigBlock) instanceField
							.get(instance);
					if (value == null) {
					...
					for (Map.Entry<String, Object> entry : ((Map<String, Object>) value
						...
						writer.write(spacing + "  " + toNodeName(key) + ":"
								+ lineSeparator);
	...
	private static Field getField(String[] split, Object instance) {
		try {
			...
			logger.info("Invalid config field: " + StringHelper.join(split, ".")
					+ " for "
