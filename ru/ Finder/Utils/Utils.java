...
public class Utils
{
    public static Location lookAt(Location loc, final Location lookat) {
        loc = loc.clone();
        final double doublex = lookat.getX() - loc.getX();
        final double doubley = lookat.getY() - loc.getY() - 1.0;
        final double doublez = lookat.getZ() - loc.getZ();
        if (doublex != 0.0) {
            if (doublex < 0.0) {
        ...
        final double doublexz = Math.sqrt(Math.pow(doublex, 2.0) + Math.pow(doublez, 2.0));
